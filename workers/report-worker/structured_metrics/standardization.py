...
def check_date_and_timestamp_format(df, imputed_columns=None):
    ...
    # Handle dynamic structures safely
    if isinstance(date_info, list):
        columns_to_validate_date = []
        expected_date_format = None

        for item in date_info:
            columns_to_validate_date.extend(item.get("column", []))
            if not expected_date_format:
                expected_date_format = item.get("format")
        ...
        columns_to_validate_date = None
        expected_date_format = None


    if isinstance(timestamp_info, list):
        columns_to_validate_timestamp = []
        expected_timestamp_format = None

        for item in timestamp_info:
            columns_to_validate_timestamp.extend(item.get("column", []))
            if not expected_timestamp_format:
                expected_timestamp_format = item.get("format")

    elif isinstance(timestamp_info, dict):
        columns_to_validate_timestamp = timestamp_info.get("column")
        expected_timestamp_format = timestamp_info.get("format")

    else:
        columns_to_validate_timestamp = None
        expected_timestamp_format = None
