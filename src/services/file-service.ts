...
export class FileService implements FileServiceInterface {
  ...
  async generatePreview(options: {
    ...
    return {
      ...
      metadata: {
        ...
        fileType,
        totalRows:
          typeof previewData === "object" && "totalRows" in previewData
            ? (previewData as any).totalRows
            : undefined,
  ...
  > {
    ...
    try {
      ...
      logger.info(
        `Attempting to get partial object for preview with key: ${key}, databankId: ${databankId}`,
      );
      ...
      try {
        ...
        const maxBytes =
          estimatedBytesPerLine > 0
            ? Math.min(
                estimatedBytesPerLine * maxLines * 1.5,
                S3Constants.MAX_PREVIEW_FILE_SIZE,
              )
            : 0;
        ...
        switch (fileType) {
          ...
          case FileTypes.XML as SupportedFileType:
            ...
            logger.info(
              "XML data processed, but structured preview is not supported by default.",
              xmlData,
            );
            result = {
              previewSupported: false,
              message:
                "Preview not supported for XML files in a structured table format.",
            };
            break;
          case FileTypes.PARQUET as SupportedFileType:
            const parquetResult = await this.processParquet(
              databankId,
              key,
              maxLines,
            );
        ...
        if (
          ...
          logger.error(
            `File not found in S3: key=${key}, databankId=${databankId}`,
          );
  ...
  private async streamToBuffer(stream: Readable): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const isNodeStream =
        typeof stream.on === "function" && typeof stream.read === "function";
  ...
  private async processJSON(
    ...
    try {
      ...
      if (
        ...
        logger.warn(
          "JSON data is not an array of objects or a single object, preview not supported as structured data.",
        );
        return {
          previewSupported: false,
          message:
            "Preview is supported for an array of JSON objects or a single JSON object. The provided JSON structure is not supported for tabular preview.",
        };
      }
    } catch (err) {
      logger.error(
   ...
   */
  private async processParquet(
    databankId: string,
    key: string,
    maxLines: number,
  ): Promise<ParquetPreviewResult> {
    logger.debug("Processing Parquet file", { maxLines });

    try {
      await this.duckdb.createConnection();
      return await new Promise(async (resolve, reject) => {
        await this.duckdb.getParquetPreview(
          databankId,
          key,
          maxLines,
          resolve,
